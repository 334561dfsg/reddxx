...
test('freeze and admin unfreeze preserve pre-existing business frozen funds', () => {
  ...
  assert.equal(frozen.accountKey, 'market')
  assert.equal(frozen.coinKey, 'USDT')

  const unfrozen = unfreezeAdminFunds(fundsInput({
    userId: user.id,
    amount: '100',
    reason: '排查完成',
    operatorId: 'admin_current'
  ...
  const fullyUnfrozen = unfreezeAdminFunds(fundsInput({
    userId: user.id,
    amount: '150.25',
    reason: '全部解冻',
    operatorId: 'admin_current'
...
test('freeze and unfreeze validate entered amounts without partial writes', () => {
  for (const amount of ['0', '-1', '1.234', '999999']) {
    ...
    assert.throws(() => freezeAllAvailable(fundsInput({ userId: user.id, amount, reason: '测试', operatorId: 'admin_current' })))
    assert.deepEqual(getFundsSnapshot(user.id), before)
    assert.equal(getFundsAuditLog({ userId: user.id }).length, auditCount)
  }

  freezeAllAvailable(fundsInput({ userId: user.id, amount: '200', reason: '测试冻结', operatorId: 'admin_current' }))
  for (const amount of ['0', '-1', '1.234', '200.01']) {
    const before = getFundsSnapshot(user.id)
    const auditCount = getFundsAuditLog({ userId: user.id }).length
    assert.throws(() => unfreezeAdminFunds(fundsInput({ userId: user.id, amount, reason: '测试', operatorId: 'admin_current' })))
    assert.deepEqual(getFundsSnapshot(user.id), before)
    assert.equal(getFundsAuditLog({ userId: user.id }).length, auditCount)
  }
})
...
test('deduction creates one transaction and failed commands are atomic', () => {
  ...
  const deductLogs = getFundsAuditLog({ userId: user.id, type: 'deduct' })
  assert.equal(deductLogs.length, 1)
  assert.equal(deductLogs[0].accountKey, 'market')
  assert.equal(deductLogs[0].accountLabel, '市币账户')
  assert.equal(deductLogs[0].coinKey, 'USDT')

  const beforeFailure = getFundsSnapshot(user.id)
  const auditCount = getFundsAuditLog({ userId: user.id }).length
  for (const amount of ['0', '-1', '1.234', '999999']) {
    assert.throws(() => deductAvailableFunds(fundsInput({
      userId: user.id,
      amount,
      reason: '无效划扣',
      operatorId: 'admin_current'
...
test('funds operations require account and coin before mutating balances', () => {
  for (const invalid of [
    { accountKey: '', coinKey: 'USDT', message: /操作账户必填/ },
    { accountKey: 'unknown', coinKey: 'USDT', message: /操作账户无效/ },
    { accountKey: 'market', coinKey: '', message: /操作币种必填/ },
    { accountKey: 'market', coinKey: 'BTC', message: /操作币种无效/ }
  ]) {
    const before = getFundsSnapshot(user.id)
    const auditCount = getFundsAuditLog({ userId: user.id }).length
    assert.throws(() => freezeAllAvailable({
      userId: user.id,
      amount: '10',
      reason: '字段校验',
      operatorId: 'admin_current',
      accountKey: invalid.accountKey,
      coinKey: invalid.coinKey
    }), invalid.message)
    assert.deepEqual(getFundsSnapshot(user.id), before)
    assert.equal(getFundsAuditLog({ userId: user.id }).length, auditCount)
  }
...
test('freeze and unfreeze reject unavailable operations without side effects', () => {
  user.balance = 0
  const before = getFundsSnapshot(user.id)
  assert.throws(() => freezeAllAvailable(fundsInput({ userId: user.id, amount: '1', reason: '测试', operatorId: 'admin_current' })), /不能超过可用余额/)
  assert.throws(() => unfreezeAdminFunds(fundsInput({ userId: user.id, amount: '1', reason: '测试', operatorId: 'admin_current' })), /没有可解冻的资金/)
  assert.deepEqual(getFundsSnapshot(user.id), before)
...
test('withdraw flow limits derive progress instead of accepting operator input', () => {
  ...
  assert.equal(active.flowScope, 'spot')
  assert.equal(active.flowScopeLabel, '现货交易')
  ...
  assert.equal(updated.flowScope, 'contract')
  assert.equal(updated.flowScopeLabel, '合约交易')
...
test('withdraw flow limit validates and persists the selected turnover scope', () => {
  const active = setWithdrawFlowLimit({
    userId: user.id,
    flowScope: 'finance',
    requiredTurnover: '800',
    expiresAt: null,
    reason: '理财收益流水要求',
    operatorId: 'admin_current'
  })

  assert.equal(active.flowScope, 'finance')
  assert.equal(active.flowScopeLabel, '理财收益')
