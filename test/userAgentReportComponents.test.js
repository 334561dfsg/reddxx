...
test('agent report Drawer renders four summary cards, every product line, and ten daily rows per page', async (t) => {
  ...
  const contentScroll = harness.findByTestId('agent-report-content-scroll')
  ...
  assert.ok(contentScroll)
  assert.ok(overview)
  assert.ok(productTable)
  assert.ok(dailyHeader)
  assert.ok(pagination)
  assert.equal(contentScroll.getAttribute('role'), 'region')
  assert.equal(contentScroll.getAttribute('aria-labelledby'), 'agent-report-content-title')
  assert.equal(contentScroll.getAttribute('tabindex'), '0')
  assert.equal(contentScroll.classList.contains('overflow-y-auto'), true)
  assert.equal(contentScroll.classList.contains('focus-visible:ring-2'), true)
  assert.equal(contentScroll.contains(summaryCards[0]), true)
  assert.equal(contentScroll.contains(productRows[0]), true)
  assert.equal(contentScroll.contains(dailyHeader), true)
  assert.equal(contentScroll.contains(renderedDailyRows[0]), true)
  assert.equal(contentScroll.contains(paginationSummary), false)
  assert.equal(pagination.contains(paginationSummary), true)
  assert.equal(productTable.classList.contains('divide-y'), true)
  ...
  assert.match(productTable.textContent, /业务量/)
...
test('agent report Drawer uses approved empty copy for each report section', async (t) => {
  ...
  assert.ok(overview)
  assert.ok(contentScroll)
  assert.match(contentScroll.textContent, /暂无代理业绩明细/)
  assert.equal(contentScroll.getAttribute('tabindex'), '0')
...
test('agent report Drawer uses one keyboard-accessible report scroller and compact fixed pagination', async (t) => {
  ...
  assert.equal(contentScroll.classList.contains('min-h-0'), true)
  assert.equal(contentScroll.classList.contains('flex-1'), true)
  assert.equal(contentScroll.classList.contains('overflow-y-auto'), true)
  assert.equal(contentScroll.getAttribute('tabindex'), '0')
})
...
test('agent report Drawer assigns bottom safe-area padding only to the flexible region without a footer', async (t) => {
  ...
  const dailyEmptyScroll = dailyEmptyHarness.findByTestId('agent-report-content-scroll')
  ...
  const pagedScroll = pagedHarness.findByTestId('agent-report-content-scroll')
...
test('agent report Drawer uses the shared fixed layer, fixed report regions, and required motion safeguards', async () => {
  ...
  assert.match(source, /data-testid="agent-report-content-scroll"/)
  ...
  assert.match(source, /agent-report-content-scroll" class="min-h-0 flex-1 overflow-y-auto/)
  assert.equal((source.match(/overflow-y-auto/g) || []).length, 3)
