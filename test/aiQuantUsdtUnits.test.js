...
test('AI quant front orders open a dedicated detail page', () => {
  ...
  assert.match(frontOrdersPanelSource, /data-testid="`ai-quant-order-card-info-\$\{o\.id\}`"/)
  assert.match(frontOrdersPanelSource, />购买时间<\/span>/)
  assert.match(frontOrdersPanelSource, />结束时间<\/span>/)
  assert.match(frontOrdersPanelSource, />支付金额<\/span>/)
  assert.match(frontOrdersPanelSource, />累计收益<\/span>/)
  assert.match(frontOrdersPanelSource, /formatOrderMoney\(o\.principal, o\.currency\)/)
  assert.match(frontOrdersPanelSource, /formatOrderMoney\(o\.accumulatedYield, o\.currency\)/)
  assert.ok(frontOrdersPanelSource.includes('grid grid-cols-2 gap-2 border-t border-white/[0.06] pt-3 md:hidden'))
