...
test('shared setting modal keeps only its body scrollable and keeps result copy compact', () => {
  ...
  assert.match(source, /:hint="selectedControlType\?\.description \|\| '请选择盈利或亏损的控制方向'"[\s\S]*id-base="user-control-strategy"/)
  assert.match(source, /:hint="selectedControlMethod\?\.description \|\| '请选择默认、做高或做低的处理方式'"[\s\S]*id-base="user-control-method"/)
  ...
  assert.match(source, /影响当前用户的交割合约订单最终结算结果/)
  assert.match(source, /订单成交并形成该用户最终交易结果时/)
  assert.match(source, /示例：用户 AI 量化周期结算时/)
