...
test('relationship drawer paginates members and resets selection and page for changed list contexts', async (t) => {
  ...
  const filterToggle = harness.findByTestId('relationship-drawer-filter-toggle')
  assert.equal(controls.contains(search), true)
  assert.equal(filterToggle.getAttribute('aria-expanded'), 'false')
  filterToggle.click()
  await harness.flush()
  const filterPanel = harness.findByTestId('relationship-drawer-filter-panel')
  ...
  assert.equal(filterPanel.classList.contains('absolute'), true)
  assert.equal(filterPanel.classList.contains('z-20'), true)
  assert.equal(controls.contains(filterPanel), true)
  assert.equal(controls.contains(statusGroup), true)
  assert.equal(controls.contains(roleGroup), true)
  assert.equal(list.contains(filterPanel), false)
  assert.equal(pagination.contains(filterPanel), false)
