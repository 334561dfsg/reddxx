...
test('operation catalog keeps the approved quick actions and grouped entries', () => {
  ...
  assert.deepEqual(agent.entries.map((entry) => entry.title), ['设置上级代理', '设置上级业务员'])
...
test('operation catalog exposes agent-only reporting and contextual agent actions', () => {
  ...
  assert.deepEqual(agent.entries.map((entry) => entry.id), ['set-agent-parent', 'staff-report', 'agent-subordinates', 'agent-report'])
  assert.deepEqual(agent.entries.map((entry) => entry.title), ['设置上级代理', '查看员工业绩', '查看下级用户', '查看代理报表'])
  ...
  assert.equal(getUserOperationEntry('set-agent-parent', { role: 'user' }).description, '设置用户归属的上级代理账号')
...
test('user list exposes one-click row actions and coordinates the complete operation drawer', () => {
  ...
  assert.match(source, /aria-label="设置用户上级代理"[\s\S]*id: 'set-agent-parent'[\s\S]*>\s*设置上级代理<\/button>/)
  ...
  assert.doesNotMatch(source, /@request-cancel="requestControlCancelFromSetting"/)
  ...
  assert.equal((actionBar.match(/class="inline-flex h-8 min-w-/g) || []).length, 11)
