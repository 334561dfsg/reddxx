...
test('operation drawer renders grouped actions and explains planned entries without mutation', () => {
  ...
  assert.match(source, /v-for="group in normalOperationGroups"/)
  ...
  assert.match(source, /data-testid="user-operation-normal-actions"/)
...
test('operation drawer uses compact, balanced action tiles without shrinking close target', () => {
  ...
  assert.match(source, /class="mb-2 text-xs font-semibold/)
  assert.match(source, /grid grid-cols-2 gap-2/)
  assert.match(source, /min-h-14[^"\n]*p-2\.5/)
  assert.match(source, /rounded-lg border border-slate-200 bg-white px-3 py-2/)
